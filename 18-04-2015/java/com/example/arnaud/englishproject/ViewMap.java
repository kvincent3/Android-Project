...
public class ViewMap {
	...
	public void LoadHistoryMap(ArrayList<DataHistory> datahistory)
	{
		Log.d("test","c'est ok");
		this.map = ((MapFragment) this.activity.getFragmentManager().findFragmentById(
				R.id.map2)).getMap();
		...
		if (this.map !=null && datahistory!=null)
		{
			...
			for (int j=0; j < datahistory.size();j++) {
				Location l=datahistory.get(j).getLocation();
				final String image = l.getMarkers().get(0).getImage();
				final String texte = datahistory.get(j).getStory();
				options.add(new MarkerOptions()
						.position(l.getMarkers().get(0).getCoor())
						.title(l.getMarkers().get(0).getTitre())
						.snippet(l.getMarkers().get(0).getTexte())
						.icon(BitmapDescriptorFactory.fromAsset(l.getMarkers().get(0).getImage())));
			} 
			for (int j=0;j<options.size();j++)
			{
				this.map.addMarker(options.get(j));
				Log.d("info",options.get(j).toString());
				...
				if (l.getZoom() != -1) {
					...
					for (int j = 0; j < l.getMarkers().size(); j++) {
						MarkerOptions tmp = new MarkerOptions()
								...
								.title(l.getMarkers().get(j).getTitre())
								.snippet(l.getMarkers().get(j).getTexte())
								.icon(BitmapDescriptorFactory.fromAsset(l
										.getMarkers().get(j).getImage()));
						map.addMarker(tmp);
						map.setOnMarkerClickListener(new GoogleMap.OnMarkerClickListener() {
							@Override
							public boolean onMarkerClick(Marker marker) {
								...
								Log.d("MARKER",
										"internal variable has been intialized");
								return false; // Conserver le comportement
												// normal
							}
						});
					}
